...
exports.getLastGameInfo = function(callback) {
    //var a = timer('getLastGameInfo');
   // var b = timer('getLastGameInfo2')
    query('SELECT MAX(id) id FROM games WHERE coin = \'btc\'', function(err, results) {
        if (err) {
            return callback(err);
        }
        ...
        if (!id || id === 0) {
            return callback(null, {
                id: 0,
                hash: '695f8ebe82aa1dff3862483ac32cb25bc4422760c733c7fe2a6f735af8452dec' 
            });
        }

       // b.stop()
        query('SELECT hash FROM game_hashes WHERE game_id = $1 AND coin = \'btc\'', [id], function(err, results) {
            if (err) {
                return callback(err);
            }
...
exports.placeBet = function(amount, autoCashOut, userId, gameId, callback) {
    ...
    getClient(function(client, callback) {
        var tasks = [
            function(callback) {
                client.query('UPDATE users SET balance_satoshis_btc = balance_satoshis_btc - $1, wagered_btc = wagered_btc + $1 WHERE id = $2', [amount, userId], callback);
            },
            function(callback) {
                client.query(
                    'INSERT INTO plays(user_id, game_id, bet, auto_cash_out, coin) VALUES($1, $2, $3, $4, $5) RETURNING id', [userId, gameId, amount, autoCashOut, 'btc'], callback);
            }
        ];
...
exports.getLocalDepositAddresses = function(callback) {
    //var j = timer('getLocalDepositAddresses');
...
exports.getBalance = function(user, callback) {
    query('SELECT balance_satoshis_btc FROM users WHERE id = $1', [user],
        function(err, results) {
            if (err) {
                return callback(err);
            }

            return callback(null, Math.floor(results.rows[0].balance_satoshis_btc / 1e6));
        });
};
...
function addSatoshis(client, userId, amount, callback) {
    ...
    amount = amount * 1e6;
    client.query('UPDATE users SET balance_satoshis_btc = balance_satoshis_btc + $1 WHERE id = $2', [amount, userId], function(err, res) {
        if (err) return callback(err);
        assert(res.rowCount === 1);
        callback(null);
    });
        ...
        addSatoshis(client, userId, amount, function(err) {
            ...
            client.query(
                ...
                function(err, result) {
                    ...
                    client.query('UPDATE users SET net_profit_btc = net_profit_btc + $1 where id = $2',[profit.toNumber(), userId]  ,function(err, result) {
                        if (err)
                            return callback(err);
...
exports.createGame = function(gameId, callback) {
    ...
    query('SELECT hash FROM game_hashes WHERE game_id = $1 and coin = \'btc\'', [gameId], function(err, results) {
        if (err) return callback(err);
        ...
        query('INSERT INTO games(id, game_crash, coin) VALUES($1, $2, $3)', [gameId, gameCrash, 'btc'], function(err) {
            if (err) return callback(err);
...
exports.getBankroll = function(callback) {
   // var g = timer('getBankroll');
    query('SELECT balance_satoshis_invested_btc FROM users WHERE balance_satoshis_invested_btc > 999999',
        function(err, results) {
            if (err) {
                err.wtf = 'getBankroll'
                return callback(err);
            ...
            for (var p in results.rows) {
                bankroll += new Decimal(results.rows[p].balance_satoshis_invested_btc).dividedBy(1e6).floor().toNumber();
            }

            //console.log("getBankroll", bankroll)
...
exports.getUserPlays = function(userId, limit, offset, callback) {
    ...
    query('SELECT p.bet, p.cash_out, p.created, p.game_id, g.game_crash FROM plays p ' +
        ...
        'WHERE p.user_id = $1 AND coin = \'btc\' ORDER BY p.id DESC LIMIT $2 OFFSET $3', [userId, limit, offset],
        function(err, result) {
            if (err) return callback(err);
...
exports.getGameHistory = function(callback) {
    ...
    //var t = timer('getGameHistory');
    var sql =
        'SELECT games.id game_id, game_crash, created, ' +
        '     (SELECT hash FROM game_hashes WHERE game_id = games.id AND coin = \'btc\'), ' +
        '     (SELECT to_json(array_agg(to_json(pv))) ' +
        '        FROM (SELECT username, bet AS stopped_at ' +
        '              FROM plays JOIN users ON user_id = users.id WHERE game_id = games.id AND coin = \'btc\') pv) player_info ' +
        'FROM games ' +
        'WHERE games.ended = true AND coin = \'btc\' ' +
...
exports.getPendingInvestments = function(callback) {
    query('SELECT * FROM investments WHERE status IS NULL AND coin = \'btc\' ORDER BY created DESC', function(err, results) {
        if (err) return callback(err);
            ...
            client.query('SELECT balance_satoshis_clam, balance_satoshis_invested_clam, username FROM users WHERE id = $1', [userId], function(err, result) {
                ...
                client.query('UPDATE users SET balance_satoshis_clam = balance_satoshis_clam - $1 WHERE id = $2', [investAmount.toNumber(), userId], function(err) {
                    ...
                    client.query('WITH sitebalance AS ( SELECT (' +
                        ...
                        ')' +
                        'INSERT INTO investments (id, user_id, username, amount, investment_balance_prev, site_balance, action, allset, coin)' +
                        'values($1,$2,$3,$4,(SELECT * FROM userbalance) , (SELECT * FROM sitebalance), $5, $6, $7)', [uid, userId, username, investAmount.floor().toNumber(), 'invest', allSet, 'clam'],
                        function(err) {
                            if (err) {
                                if (err.code === '23505') { // dupe key
                                    return callback('INVESTMENT_ALREADY_MADE');
...
exports.queueWeeklyCommission = function(callback) {
    query('SELECT * FROM weekly_commissions WHERE completed IS NULL AND coin = \'btc\'', function(err, results) {
        if (results.rowCount === 0) {
            query('INSERT into weekly_commissions (status, coin) values($1, $2)', ['QUEUED', 'btc'], function(err) {
...
exports.takeWeeklyCommission = function(commissionId, callback) {
    ...
    query('SELECT bankrole_profit_btc, hightide_btc, username, id, balance_satoshis_invested_btc FROM users WHERE balance_satoshis_invested_btc > 99999999', function(err, results) { //bankrole_profit > hightide', function(err, results) {
        if (err) {
            logger.info(err)
            return callback(err);
        }
        ...
        for (var n = 0; n < results.rows.length; n++) {
            ...
            var investedBal = new Decimal(entry.balance_satoshis_invested_btc).dividedBy(1e14).toFixed(8)
            investedBalances.push(Number(investedBal))

            var br = new Decimal(entry.bankrole_profit_btc)
            var ht = new Decimal(entry.hightide_btc)
        ...
        async.eachSeries(userList, function(user, callback) {
            ...
            var commissionable = bigInt('' + user.bankrole_profit_btc).subtract('' + user.hightide_btc)
            var commission = Number(bigInt(commissionPercentage).multiply(commissionable).divide(100))
            var hightide = user.bankrole_profit_btc;
            ...
            getClient(function(client, callback) {
                client.query('UPDATE users SET hightide_btc = $1, balance_satoshis_invested_btc = balance_satoshis_invested_btc - $2 where id = $3', [hightide, commission, user.id], function(err, result) {
                    if (err) return callback(err)


                    client.query('INSERT into commissions (id, user_id, amount, reason, from_user, coin) values($1, 7, $2, $3, $4, $5)', [uuid.v4(), commission, 'weekly-commission', user.id, 'btc'], function(err, result) {
                        if (err) return callback(err)
                        ...
                        client.query('WITH sitebalance AS ( SELECT (' +
                            ...
                            '  )' +
                            ')' +
                            'INSERT INTO investments (id, user_id, username, investment_balance_prev, site_balance, action, status, commission_amount, amount, coin)' +
                            'values($1,$2,$3,$4,(SELECT * FROM sitebalance), $5, $6, $7, 0, $8)', [uuid.v4(), user.id, user.username, user.bankrole_profit_btc, 'weekly-commission', 'complete', commission, 'btc'],
                            function(err, result) {
                                if (err) return callback(err)
            ...
            query('UPDATE weekly_commissions SET completed = now(), users = $1, amount = $2, status = $3 where id = $4', [totalUsers, totalComission.toNumber(), 'COMPLETE', commissionId], function(err) {
                ...
                query('UPDATE users SET balance_satoshis_btc = balance_satoshis_btc + $1 WHERE id =7', [totalComission.toNumber()], function(err) {
                    if (err) return callback(err)
...
exports.endGame = function(gameId, givenOut, totalBet, instaCrash, callback) {
    ...
    getClient(function(client, callback) {
        client.query('UPDATE games SET ended = true WHERE id = $1', [gameId],
            ...
            client.query('UPDATE site_stats SET wagered_btc =  wagered_btc + $1, net_profit_btc = net_profit_btc + $2', [totalBetPercision ,roundTotal],
            function(err) {
                if (err) return callback(new Error('Could not end game, got: ' + err));
                ...
                client.query('SELECT COALESCE(SUM(balance_satoshis_invested_btc),0) as total FROM users', function(err, result) {
                    if (err) return callback(err)

                    var onsite = result.rows[0].total
                    client.query('SELECT * FROM users WHERE balance_satoshis_invested_btc > 999999', function(err, results) {
                        if (err) return callback(err)
                        ...
                        async.eachSeries(results.rows, function(user, callback) {
                            ...
                            var ubs = new Decimal(user.balance_satoshis_invested_btc.toString()).times(1e6)
                            ...
                            var newBankrollProfit  =  new Decimal(user.bankrole_profit_btc.toString()).plus(totalCut)      
                            var newInvestedBalance =  new Decimal(user.balance_satoshis_invested_btc.toString()).plus(totalCut.toString())      

                            totalProfitDifference   = new Decimal(totalProfitDifference).plus(newInvestedBalance).minus(user.balance_satoshis_invested_btc.toString())
                            totalBankrollDifference = new Decimal(totalBankrollDifference).plus(newBankrollProfit).minus(user.bankrole_profit_btc.toString())

                            client.query('UPDATE users SET balance_satoshis_invested_btc = $1, bankrole_profit_btc = $2 WHERE id = $3', [newInvestedBalance.toNumber(), newBankrollProfit.toNumber(), user.id], function(err, results) {
                                if (err) return callback(err)
                            ...
                            logger.info("BTC: GameId (" + gameId + ") " + crashString, "totalBet:", totalBet, "givenOut:", totalProfitDifference.dividedBy(1e6).round().toString(), "difference:", totalBankrollDifference.toString(), " (", userCount, " users )");
                            callback()
                        });
                    });
                });
...
exports.invest = function(uid, userId, amount, callback) {
    getClient(function(client, callback) {
        client.query('UPDATE users SET balance_satoshis_invested_btc = balance_satoshis_invested_btc + $1 where id = $2', [amount, userId], function(err, data) {
            if (err) {
                if (err.code === '23514') { // constraint violation
                    return callback('NOT_ENOUGH_BALANCE');
                }
...
exports.divest = function(uid, userId, amount, allSet, callback) {
    getClient(function(client, callback) {
        client.query('SELECT bankrole_profit_btc, balance_satoshis_invested_btc, hightide_btc FROM users where id = $1', [userId], function(err, data) {
            if (err) {
                logger.error(err)
                return callback(err);
            }
            ...
            client.query('SELECT * FROM investments where id = $1', [uid], function(err, investmentResult) {
                ...
                var hightide = new Decimal(data.rows[0].hightide_btc);
                var balanceInvested = new Decimal(data.rows[0].balance_satoshis_invested_btc);
                var bankroleProfit = new Decimal(data.rows[0].bankrole_profit_btc); //balance_satoshis_invested;          
                ...
                if (commission.greaterThan(0)) {
                    ...
                    if (balanceInvested.minus(total).equals(0)) {
                        ...
                        tasks.push(function(callback) {
                            client.query('UPDATE users SET hightide_btc = $1, balance_satoshis_btc = balance_satoshis_btc + $2, balance_satoshis_invested_btc = balance_satoshis_invested_btc - $3 where id = $4', [hightide.toNumber(), total.minus(commission).toNumber(), total.toNumber(), userId], callback)
                        })
                        tasks.push(function(callback) {
                        ...
                        tasks.push(function(callback) {
                            client.query('UPDATE users SET hightide_btc = $1, balance_satoshis_invested_btc = 0,  balance_satoshis_btc = balance_satoshis_btc + $2 where id = $3', [hightide.toNumber(), total.toNumber(), userId], callback)
                        })
                        tasks.push(function(callback) {
                        ...
                        tasks.push(function(callback) {
                            client.query('UPDATE users SET hightide_btc = $1, balance_satoshis_invested_btc = $2,  balance_satoshis_btc = balance_satoshis_btc + $3 where id = $4', [hightide.toNumber(), balanceInvested.toNumber(), total.toNumber(), userId], callback)
                        })
                        tasks.push(function(callback) {
                            client.query(investSql, ['complete', commission.toNumber(), total.toNumber(), uid, investmentBalancePrev.toNumber()], callback)
                        })
                    ...
                    tasks.push(function(callback) {
                        client.query('UPDATE users SET balance_satoshis_btc = balance_satoshis_btc + $1 WHERE id = 7', [commission.toNumber()], callback)
                    })
                    tasks.push(function(callback) {
                        var uid2 = uuid.v4();
                        client.query('INSERT INTO commissions(id, user_id, amount, reason, from_user, coin) VALUES($1, $2, $3, $4, $5, $6)', [uid2, 7, commission.toNumber(), "divest-gambling-commission", userId, 'btc'], callback)
                    })


                } else {
                    tasks.push(function(callback) {
                        client.query('UPDATE users SET  balance_satoshis_btc = balance_satoshis_btc + $1, balance_satoshis_invested_btc = balance_satoshis_invested_btc - $1 where id = $2', [total.toNumber(), userId], callback)
                    })
                    tasks.push(function(callback) {
        ...
        client.query('SELECT * FROM investments WHERE user_id = $1 AND status IS NULL AND coin = \'btc\'', [userId], function(err, data) {
            ...
            client.query('SELECT balance_satoshis_btc, balance_satoshis_invested_btc, username FROM users WHERE id = $1', [userId], function(err, result) {
                ...
                if (divest === "all") {
                    ...
                    divestAmount = new Decimal(divest).times(1e14) 
...
exports.checkPrizePoolPayout = function(callback) {
    ...
    getClient(function(client, callback) {
        client.query('SELECT * FROM weekly_prize_log WHERE week = $1 and year = $2 AND coin = \'btc\'', [week, year], function(err, result) {
            if (err) return callback(err)

            if (result.rows.length === 0) {
                client.query('SELECT * FROM weekly_prize_log WHERE week = $1 and year = $2 AND coin = \'btc\'', [previousWeek, year], function(err, results) {
                    if (err) return callback(err)
                    ...
                    async.parallel({
                            ...
                            for (var h = 0; h < results.net.rows.length; h++) {
                                ...
                                if (entry.net_rank <= 10) {
                                    ...
                                    var annon = function(weeklyPrize, callback) { 
                                        tasks.push(function(callback) {
                                            client.query('INSERT INTO weekly_prizes (user_id, amount, week, year, prize_type, coin) values($1,$2,$3,$4,$5,$6)', [weeklyPrize.userId, weeklyPrize.prizeAmount, weeklyPrize.week, weeklyPrize.year, weeklyPrize.type, 'btc'], callback)
                                        })
                        ...
                        Object.keys(payouts).forEach(function(user) {
                            tasks.push(function(callback) {
                                client.query('UPDATE users SET balance_satoshis_btc = balance_satoshis_btc + $1 WHERE id = $2', [payouts[user], parseInt(user)], callback)
                            })
                        })
                        ...
                        if(total.greaterThan(0)) {
                            tasks.push(function(callback) {
                                client.query('INSERT INTO weekly_prize_log (week, year, status, total, clam) values($1,$2,$3,$4,$5)', [previousWeek, year, 'complete', total.toNumber(), 'btc'], callback)
                            })
                        }
