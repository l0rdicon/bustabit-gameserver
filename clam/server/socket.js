...
var limiter = RateLimiter({
    redis: client,
    no_ready_check: true,
    namespace: "UserStatsLimiterClam",
    interval: 1000,
    maxInInterval: 2,
    minDifference: 100
});
...
module.exports = function(server, game, chat, depositAddresses, userList, staker) {
    ...
    chat.on('modmsg', function(msg) {
         for (var i=0; i < clients.length; i++) {
            clients[i].send(JSON.stringify({type:"mod", msg:msg}));
        };
        ...
        if (loggedIn) {
            ...
            socket.on('place_bet', function(amount, autoCashOut, ack) {
                ...
                if (loggedIn.frozen === true)
                    return sendError(socket, 'Your account is locked. Please contact an https://freebitcoins.supportsystem.com/');
                ...
                if (new Date().getTime() - lastMessage[loggedIn.id] > 1000) {
                    ...
                    if (cmdMatch) {
                        ...
                        switch (cmd) {
                                    ...
                                    database.queueWeeklyCommission(function(err) {
                                        if (err) {
                                            ...
                                            socket.emit('msg', {
                                                time: new Date(),
                                                type: 'info',
                                                message: 'Weekly clam commissions have been queued'
                                            });
                                        }
                                    })
                                }
                            ...
                            case 'reward':
                                ...
                                if (rewardMatch) {
                                    ...
                                    database.getWeeklyDonated(week, year, rewardStatsMatch[2], function(err, currentTotal) {
                                        ...
                                        tipEvent(loggedIn.id, "The current prize pool for the weekly leaderboard is " + currentTotal + " " + rewardStatsMatch[2]);
                                    })
                                } else if (donorsMatch) {
                                    var week = currentWeekNumber();
                                    var year = new Date().getFullYear();
                                    ...
                                    database.getWeeklyDonors(year, week, donorsMatch[2], function(err, donorList) {
                                        ...
                                        for (var i = 0; i < donorList.length; i++) {
                                            msg += " (" + donorList[i].user_id + ")" + " " + userList[donorList[i].user_id] + " " + new Decimal(donorList[i].amount).dividedBy(1e14).toFixed(8) + " " + donorsMatch[2]
                                                ...
                                                case (data.sendto.match(/^(?:\d+(?:,\d+)*)?$/) || {}).input:
                                                    if (!data.divy) {
                                                        ...
                                                        database.tipUserByIds(loggedIn.id, ids, data.amount, data.divy, data.coin, function(err, result) {
                                                            ...
                                                            if (data.privatetip === "private")
                                                                sendPrivateTipInfo(loggedIn.id, "(" + loggedIn.id + ") " + loggedIn.username + " sent " + tipStr + "for a total of " + total + " " + data.coin.toUpperCase());
                                                            else
                                                                sendTipInfo("(" + loggedIn.id + ") " + loggedIn.username + " sent " + tipStr + "for a total of " + total + " " + data.coin.toUpperCase());

                                                            tipEvent(loggedIn.id, "You sent " + tipStr + "for a total of " + total + " " + data.coin.toUpperCase());
                                                        });
                                                    }
                            ...
                            case 'divest':
                                if (loggedIn.frozen === true)
                                    return sendErrorChat(socket, 'Your account is locked. Please contact https://freebitcoins.supportsystem.com');
                            ...
                            case 'shutdown':
                                if (loggedIn.admin) {
                                    ...
                                    chat.emit('msg', {
                                        time: new Date(),
                                        type: 'info',
                                        message: "Clam Server shutting down..."
                                    });
                                } else {
